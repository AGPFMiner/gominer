...
const (
	//HashSize is the length of a keccak hash
	HashSize = 32
)
...
func (sc *StratumClient) subscribeToStratumDifficultyChanges() {
	sc.stratumclient.SetNotificationHandler("mining.set_difficulty", func(params []interface{}, result interface{}) {
		diffStr, ok := params[0].(string)
		if !ok {
			log.Print("invalid target string")
			return
		}

		log.Println("Stratum server changed diff to", diffStr)
		stratumDiff, _ := strconv.ParseInt(diffStr, 16, 64)
		sc.Difficulty = float64(stratumDiff)
...
func (sc *StratumClient) subscribeToStratumJobNotifications() {
	sc.stratumclient.SetNotificationHandler("mining.notify", func(params []interface{}, result interface{}) {
		...
		challengeHash, ok := params[1].(string)
		_, err := hex.DecodeString(challengeHash)
		if !ok || err != nil {
			log.Print("invalid challengeHash")
			return
		}

		poolEthAddr, ok := params[2].(string)
		if !ok {
			log.Print("invalid poolEthAddr")
			return
		}

		diff, ok := params[3].(string)
		if !ok {
			log.Print("invalid diff")
			return
		}
		...
		sj.headerHash = challengeHash + poolEthAddr
		sj.jDiff, _ = strconv.ParseInt(diff, 16, 64)
		sj.CleanJobs = cleanJob
		sj.ExtraNonce2.Size = sc.nonce2Size - 8 //fpga returns 8 bytes
...
func (sc *StratumClient) SubmitHeader(nonce []byte, job interface{}) (err error) {
	...
	stratumUser := sc.User
	jobID := sj.JobID
	nonce2Str := hex.EncodeToString(append(sj.ExtraNonce2.Bytes(), stratum.RevBytes(nonce[:])...))
